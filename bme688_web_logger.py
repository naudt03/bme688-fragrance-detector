...
<head>
  ...
  <style>
    body { font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; margin: 18px; background:#fafafa; }
    .row { display: flex; gap: 12px; flex-wrap: wrap; align-items: end; }
    .card { border: 1px solid #e5e5e5; border-radius: 14px; padding: 14px; background: white; box-shadow: 0 1px 2px rgba(0,0,0,0.04); }
    label { display:block; font-size: 12px; color:#444; margin-bottom: 4px; }
    input { padding: 9px; border-radius: 12px; border: 1px solid #d0d0d0; min-width: 220px; background:#fff; }
    button { padding: 10px 12px; border-radius: 12px; border: 1px solid #cfcfcf; background: #f7f7f7; cursor: pointer; }
    button.primary { border-color: #333; background:#111; color:#fff; }
    button.danger { border-color: #b00; background:#fff5f5; }
    button:active { transform: translateY(1px); }

    .muted { color:#666; font-size: 13px; }
    .sep { height: 14px; }
    .pill { display:inline-flex; align-items:center; gap:6px; padding:4px 10px; border:1px solid #d6d6d6; border-radius:999px; font-size:12px; color:#333; background:#fff; }
    .grid { display:grid; grid-template-columns: 1fr 1fr; gap:12px; }
    @media (max-width: 900px){ .grid{grid-template-columns:1fr;} input{min-width: 180px;} }

    /* New: main two-column live area */
    .liveGrid { display:grid; grid-template-columns: 1fr 1.2fr; gap: 12px; align-items: start; }
    @media (max-width: 900px){ .liveGrid{grid-template-columns:1fr;} }

    /* Detection card */
    .detWrap { display:flex; flex-direction:column; gap:10px; }
    .detTitle { font-size: 13px; color:#666; margin:0; }
    .detName { font-size: 30px; font-weight: 800; letter-spacing: -0.02em; margin: 0; }
    .detSub { font-size: 14px; color:#444; margin: 0; }
    .detMeta { display:flex; flex-wrap:wrap; gap:8px; margin-top:6px; }
    .badge { display:inline-flex; align-items:center; gap:6px; padding:4px 10px; border-radius:999px; border:1px solid #ddd; font-size:12px; background:#fafafa; }
    .badge.good { border-color:#bfe3bf; background:#f1fbf1; }
    .badge.warn { border-color:#f2d7a6; background:#fff8eb; }
    .badge.bad { border-color:#f0b9b9; background:#fff3f3; }
    .bigPct { font-size: 52px; font-weight: 900; margin: 0; line-height: 1; }
    .pctRow { display:flex; gap:16px; align-items: baseline; flex-wrap: wrap; }

    /* Right-side raw panel */
    #status { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; white-space: pre; font-size: 12px; background:#0b0c10; color:#eaeaea; border-radius: 12px; padding: 12px; overflow:auto; max-height: 360px; }
    canvas { max-width: 100%; }
  </style>
</head>
<body>
  <h2 style="margin:0 0 6px 0;">BME688 Web Logger <span class="pill">Sticky + Plateau</span></h2>
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="row">
        ...
        <!-- Removed John Varvatos preset button per request -->
  ...
  <!-- NEW layout: left = fragrance card, right = chart + raw data -->
  <div class="liveGrid">
    <div class="card detWrap">
      <p class="detTitle">Now Detected</p>
      <h1 class="detName" id="detName">—</h1>

      <div class="pctRow">
        <p class="bigPct" id="detPct">--%</p>
        <div>
          <p class="detSub" id="detSub">Waiting for live data…</p>
          <div class="detMeta" id="detMeta"></div>
        </div>
      </div>

      <div class="muted" id="detExtra"></div>
    </div>

    <div class="card">
      <div style="display:flex; align-items:baseline; justify-content:space-between; gap:10px;">
        <div>
          <div style="font-weight:700;">Gas (median)</div>
          <div class="muted">Chart + raw stream (debug)</div>
        </div>
        <div class="pill" id="ohmsPill">— Ω</div>
      </div>
...
const detPct = document.getElementById("detPct");
...
const ohmsPill = document.getElementById("ohmsPill");
...
function updateDetectionUI(m){
  // Use final prediction if present, else fall back to raw
  const label = m.live_pred ?? m.live_raw_label ?? "—";
  const conf = (m.live_conf ?? m.live_raw_conf);
  detName.textContent = label || "—";
  detPct.textContent = fmtPct(conf);

  // subtitle line
  let sub = "";
  if(!m.live_enabled){
    sub = "Live classifier is OFF";
  } else if(m.live_raw_label === null || m.live_raw_label === undefined){
    sub = "Building window…";
  } else {
    sub = "Live classification";
  }
  ...
  detMeta.innerHTML = "";
  if(m.live_enabled){
    const u = m.unknown_thr ?? 0.7;
    if(label === "UNKNOWN") detMeta.appendChild(badge("UNKNOWN", "warn"));
    else if(typeof conf === "number" && conf >= u) detMeta.appendChild(badge("CONFIDENT", "good"));
    else detMeta.appendChild(badge("LOW CONF", "warn"));

    if(m.plateau_latched) detMeta.appendChild(badge("PLATEAU", "good"));
    if(typeof m.sticky_left_s === "number" && m.sticky_left_s > 0) detMeta.appendChild(badge("STICKY " + Math.round(m.sticky_left_s) + "s", "good"));
  } else {
    detMeta.appendChild(badge("OFF", "bad"));
  }

  // extra detail
  const raw = m.live_raw_label ? `${m.live_raw_label} (${fmtPct(m.live_raw_conf)})` : "—";
  const ohms = m.gas_med ? `${m.gas_med.toLocaleString()} Ω` : "— Ω";
  detExtra.textContent =
    `Raw: ${raw}  |  Gas: ${ohms}  |  Temps: ${(m.heater_temps || []).join(",")}`;
}
...
function connect(){
  ...
  es.onmessage=(e)=>{
    ...
    // keep raw JSON on right
    status.textContent=JSON.stringify(m,null,2);

    // update ohms pill
    if(m.gas_med){
      ohmsPill.textContent = m.gas_med.toLocaleString() + " Ω";
      xs.push(xs.length);
      ys.push(m.gas_med);
